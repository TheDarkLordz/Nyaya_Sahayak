...
Your response MUST be a valid JSON object conforming to this structure, and nothing else. Do not include any explanatory text before or after the JSON.`,
});
...
const suggestRelevantLawsFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (!output) {
      // Handle cases where the prompt might return nothing that fits the schema
      // or if the model response was blocked/empty before parsing.
      // This ensures we always return a valid structure, even if empty.
      return { suggestions: [] };
    }
    return output;
  }
